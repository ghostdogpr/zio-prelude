...
object ZPureSpec extends DefaultRunnableSpec {
  ...
  def spec: ZSpec[Environment, Failure] =
    suite("ZPureSpec")(
        ...
        suite("methods")(
          testM("|||") {
            check(genInt, genInt, genInt, genInt, genInt) { (s0, s1, s2, a1, a2) =>
              val z1 = ZPure.fromFunction[Unit, Int](_ => a1).asState(s1)
              val z2 = ZPure.fromFunction[Unit, Int](_ => a2).asState(s2)
          ...
          testM("filterOrElse") {
            check(genInt, genInt, genInt, genInt, genInt) { (s1, s2, s3, a1, a2) =>
              val z = ZPure.succeed(a1).asState(s2)
          ...
          testM("filterOrElse_") {
            check(genInt, genInt, genInt, genInt, genInt) { (s1, s2, s3, a1, a2) =>
              val z1 = ZPure.succeed(a1).asState(s2)
          ...
          testM("head") {
            check(genInt, genString, genString) { (s, el, el2) =>
              val optOrHead = ZPure.succeed(List(el, el2)).head.getState.either.runResult(s)
          ...
          testM("head (empty List)") {
            check(genInt) { s =>
              val optOrHead = ZPure.succeed(List.empty).head.getState.either.runResult(s)
          ...
          testM("negate") {
            check(genInt) { s =>
              assert(State.succeed(true).negate.run(s))(equalTo((s, false))) &&
              assert(State.succeed(false).negate.run(s))(equalTo((s, true)))
            }
          },
          suite("repeatN")(
            test("success") {
          ...
          testM("unit") {
            check(genInt, genInt) { (s, a) =>
              assert(State.succeed(a).unit.run(s))(equalTo((s, ())))
            }
          },
          testM("zip") {
      ...
      suite("failure")(
        suite("methods")(
          ...
          suite("none")(
            testM("success") {
              check(genInt) { s =>
                assert(ZPure.succeed(None).none.getState.either.runResult(s))(
                  isRight(equalTo((s, ())))
                )
              }
            },
            testM("failure") {
              check(genInt, genInt) { (s, a) =>
                assert(ZPure.succeed(Some(a)).none.getState.either.runResult(s))(isLeft(isNone))
              }
            }
          ),
          testM("orElseFail") {
          ...
          suite("fold")(
            ...
            testM("success") {
              ...
              check(genInt, genInt, genIntToInt, genIntToInt) { (s1, a1, failure, success) =>
                val (s2, a2) = ZPure.succeed(a1).fold(failure, success).run(s1)
      ...
      suite("log")(
        test("log example") {
          val computation: ZPure[String, Any, Nothing, Any, Nothing, Int] = for {
            a <- ZPure.succeed(1 + 1)
            _ <- ZPure.log("plus")
            b <- ZPure.succeed(a * 3)
            _ <- ZPure.log("times")
